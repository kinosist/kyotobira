...
<form action="/lockinfo/edit/<?php echo $lockinfo["id"]; ?>" method="post"><input name="_token" type="hidden" value="<?php echo csrf_token(); ?>" />    <div class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
                <div class="checkbox">
                    <input class="check-box" data-val="true" data-val-required="有効 フィールドが必要です。" id="Enable" name="enabled" type="checkbox" value="1" <?php if($lockinfo["enabled"]){ ?>checked="checked" <?php } ?>/>
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
				<select class="form-control" id="Status" name="deviceid">
					...
					<option value="<?php echo $device->id; ?>" <?php if($device->id==$lockinfo["deviceid"]){ ?>selected="selected"<?php } ?>><?php echo $device->devicename; ?></option>
					<?php }	?>
				</select>
            </div>
        </div>
